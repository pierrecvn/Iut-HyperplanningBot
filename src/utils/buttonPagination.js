...
module.exports = async (interaction, pages, time = 30 * 1000) => {
	try {
		if (!interaction || !pages || !pages.length > 0) throw new Error('Arguments invalides');
		...
		mc.on('collect', async (i) => {
			if (i.user.id !== interaction.user.id) return i.reply({ content: 'Vous \'êtes pas autorisé à faire cela !', ephemeral: true });
