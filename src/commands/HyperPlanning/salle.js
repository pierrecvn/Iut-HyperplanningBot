const { SlashCommandBuilder, StringSelectMenuBuilder, PermissionFlagsBits, ActionRowBuilder, EmbedBuilder, AttachmentBuilder } = require('discord.js');


const getIcalUrlClass = require('../../utils/getIcalUrlClass.js');
const getClass = require('../../utils/salleInfo.json');
...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('salle')
		.setDescription('Vous renvoie votre l\'emploi du temps de la salle demandée')
		...
		// 	.setName("libre")
		// 	.setDescription("Renvoi les salles libres")
	...
	run: async (client, interaction) => {
		...
		if (subs === "libre") {
			...
			interaction.replu({ content: "En cours de développement", ephemeral: true });
		}


		if (!["libre", "salle"].includes(subs)) {
			...
			const Select = new StringSelectMenuBuilder()
				...
				.setPlaceholder("Choisissez votre salle")
				.addOptions(
					choices.map(choice => ({
						label: choice.substring(0, 3),
						value: choice
					}))
			...
			interaction.reply({ content: "Veuillez la salle que vous voulez", components: [row] });
		}
	}
}
		...
		switch (subcmd) {
			case "texte":
				events = Object.values(data)
					.filter(ev => ev.type === 'VEVENT' && new Date(ev.start).setHours(0, 0, 0, 0) === now.setHours(0, 0, 0, 0));
				...
				events.forEach((ev, index) => {
					...
					if (index === events.length - 1) {
						...
						const row = new ActionRowBuilder().addComponents(button, buttonCancel);
			...
			case "image":
				...
				events = Object.values(data).filter(ev => ev.type === 'VEVENT' && new Date(ev.start).setHours(0, 0, 0, 0) === now.getTime());
