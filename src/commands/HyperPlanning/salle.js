...
module.exports = {
	...
	run: async (client, interaction) => {
		...
		if (nbSalle) {
			...
			// Tri des événements par leur heure de début
			...
			for (const [index, ev] of events.entries()) {
				...
				if (now > lastEventEnd) {
					...
					finJourneeMessage = `Salle libre !`;
				} else {
					// Sinon, affichez le temps restant jusqu'à la fin de la journée
					finJourneeMessage = `Salle libre dans : ${formatDuration(lastEventEnd - now)}`; // timeRemainingString;
				}
